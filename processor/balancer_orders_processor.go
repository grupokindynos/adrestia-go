...
func (bp *BalancerOrderProcessor) handlerBalancerOrdersExchangeDepositSent(wg *sync.WaitGroup) {
	...
	for _, order := range balancerOrders {
		exchange, err := exchangeFactory.GetExchangeByName(order.Exchange)
		if err != nil {
...
func (bp *BalancerOrderProcessor) handlerBalancerOrdersPlutusDeposit(wg *sync.WaitGroup) {
	...
	for _, order := range balancerOrders {
		...
		res, err := blockExplorer.GetTx(order.WithdrawTxId)
		if err != nil {
			log.Println("Error while getting tx from blockbook " + err.Error())
			continue
		}
		if res.Confirmations > 0 {
			receivedAmount, err := getPlutusReceivedAmount(res, order.WithdrawAddress)
			if err != nil {
